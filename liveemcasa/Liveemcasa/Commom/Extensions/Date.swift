...
public extension Date {
  ...
  var isMoreFiveYears: Bool {
    let components = Calendar.current.dateComponents([.year], from: self, to: Date())
    
    guard let differenceYear = components.year else {
      return false
    }
    
    return differenceYear >= 5 ? true : false
  }
  
  var isMoreTenYears: Bool {
    let components = Calendar.current.dateComponents([.year], from: self, to: Date())
    ...
    return differenceYear >= 10 ? true : false
  }
  ...
  func validateYears(year: Int) -> Bool {
  ...
  //Return date interval in Double
  func intervalSecondsInDouble() -> Double {
    let timeInterval = self.timeIntervalSince1970
    let dateInt = Int(timeInterval)
    let doubleDate = Double(dateInt)
    return doubleDate
  }
